...
1. **Infer the flags from the user's request** (`$ARGUMENTS`). Map natural language to flags:
   ...
   - "strict/normal redaction" → `--preset strict|normal` (default is `strict`). "loose", "light", or "minimal" redaction are all `normal`. If the user asks for "no redaction", "raw", or "unredacted", explain that Quire always redacts server-side (a hard security boundary, not a setting) and offer `normal` as the loosest preset — do not pass `--preset none`.
   - If the user named a specific session id, use it as the positional argument; otherwise share the current session.
   - Append `--yes` so the CLI never stops on a confirmation prompt.
